...
public class JpaScheduleDao implements ScheduleDao {
    ...
    @Override @Transactional(readOnly = true) public List<Schedule> findBetween(LocalDate from, LocalDate to) {
        return entityManager.createQuery("select s from Schedule s where s.eventDate <= :to and coalesce(s.endDate, s.eventDate) >= :from order by s.eventDate, s.startTime, s.id", Schedule.class)
                .setParameter("from", from).setParameter("to", to).getResultList();
    }
    @Override @Transactional(readOnly = true) public List<Schedule> findUpcoming(LocalDate from, int limit) {
        return entityManager.createQuery("select s from Schedule s where coalesce(s.endDate, s.eventDate) >= :from order by s.eventDate, s.startTime, s.id", Schedule.class)
