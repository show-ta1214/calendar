...
class CalendarControllerTest {
    ...
    @Test void scheduleCanBeCreatedEditedAndDeleted() throws Exception {
        mockMvc.perform(post("/schedules").param("title","打ち合わせ").param("eventDate","2026-08-18").param("startTime","10:00").param("endTime","11:00").param("color","sage"))
        ...
        mockMvc.perform(post("/schedules/{id}",schedule.getId()).param("title","定例会議").param("eventDate","2026-08-19").param("startTime","13:00").param("endTime","14:00").param("color","coral"))
    ...
    @Test void invalidScheduleReturnsForm() throws Exception { mockMvc.perform(post("/schedules").param("title","").param("eventDate","2026-08-18").param("startTime","11:00").param("endTime","10:00")) .andExpect(status().isOk()).andExpect(view().name("schedule-form")).andExpect(content().string(containsString("予定名を入力してください。"))).andExpect(content().string(containsString("終了時刻は開始時刻より後にしてください。"))); }
}
